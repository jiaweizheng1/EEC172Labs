...
*/
    ...
    //put the command byte on the line
    MAP_SPIDataPut(GSPI_BASE, c);
    MAP_SPIDataGet(GSPI_BASE, &dummy);

    //disable SPI protocol for OLED so it dont receive garbage
    MAP_SPIDisable(GSPI_BASE);
    MAP_SPICSDisable(GSPI_BASE);

    //write high to OLEDCS; reset to default state
