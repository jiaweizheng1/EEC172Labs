...
void rpsbuttons()
{
    if (times_index == 34)
    {
        ...
        case two:
            Message("Two\n\r");
            // red board chosed paper, then send over UART1 to tell yellow board that red has chosen paper
            if (rpstable[0].entry == ' ')
            {
                rpstable[0].entry = 'P';
        ...
        case three:
            Message("Three\n\r");
            // red board chosed scissors, then send over UART1 to tell yellow board that red has chosen scissors
            if (rpstable[0].entry == ' ')
            {
                rpstable[0].entry = 'S';
        ...
        case volumedown: // change game
            Message("Volumedown\n\r");
            gamemode += 1;
            // tell yellow board to also change game
            while (UARTBusy(UARTA1_BASE))
                ;
            UARTCharPut(UARTA1_BASE, '-');
            break;
...
static int http_post(int iTLSSockID)
{
    ...
    // get number of digits in red wins and yellow wins to pass into datalength.
    i = 0;
    ...
    // the comma separated struct is "red_wins, yellow_wins"
    // add red wins to json to be sent to AWS IOT
    for (n = 0; n < i; n++)
    {
    ...
    strcpy(pcBufHeaders, buf_ptr);
    pcBufHeaders += 1;
    // then add yellow wins to json to be sent to AWS IOT
    for (n = 0; n < j; n++)
    {
...
static int http_get(int iTLSSockID)
{
    ...
    else
    {
        ...
        // get red wins from received json from AWS IOT
        ...
        // get yellow wins from received json from AWS IOT
